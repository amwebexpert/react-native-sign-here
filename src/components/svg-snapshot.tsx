...
const SvgSnapshot: FunctionComponent<SvgSnapshotProps> = ({
  ...
  if (elements.length === 0) {
    console.warn('[SvgSnapshot]: no elements provided');
    return null;
  }

  if (!onBase64Generated) {
    console.warn('[SvgSnapshot]: onBase64Generated callback not provided');
    return null;
  }
  ...
  const onRefUpdate = (ref?: SvgRef | null) => {
    ...
    if (Platform.OS === 'ios') {
      ...
      console.log('[SvgSnapshot] onRefUpdate', { elements: elements.length, scale, width: ref.props.width });
    }
